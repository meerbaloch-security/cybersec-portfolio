...
Active Directory Domain Services, DNS, and domain controller
...
Domain-joined endpoint and monitored host
...
Authorized security testing and administration
...
Central log collection, searching, dashboards, and alerts
...
Sysmon was installed and verified on WS01. Its operational event channel was added to the Universal Forwarder input configuration. The forwarder service account, NT SERVICE\SplunkForwarder, was added to the local Event Log Readers group so it could read the Sysmon channel.
...
Local test-user creation and deletion (4720, 4726)

Scheduled-task creation and deletion (4698, 4699)

Windows service installation (7045)

PowerShell process execution through Sysmon (1)
...
Identifies PowerShell process creation on WS01

User Account Created or Deleted

Security Event IDs 4720, 4726

Detects local account lifecycle activity

Windows Service Installed

System Event ID 7045

Detects installation of a Windows service

Scheduled Task Created or Deleted

Security Event IDs 4698, 4699

Detects scheduled-task lifecycle activity

Failed Authentication Detection

Security Event IDs 4625, 4776
...
The security-monitoring dashboard contains panels for the four new detection reports alongside authentication monitoring. Two scheduled alerts were enabled:
...
These detections are intentionally broad for a small lab. In production they should be tuned with host roles, approved administrator accounts, known automation, software-deployment tools, maintenance windows, command-line context, and historical baselines.
...
Administration, logon scripts, management agents

Review user, parent process, command line, host, and related network activity

Account changes

Help-desk work, onboarding, test accounts

Validate requester, target account, actor privileges, and nearby group changes

Service installation

Software installation, updates, endpoint tools

Review service name, binary path, signer, installer process, and change ticket

Scheduled-task changes

Maintenance, backup, application updaters

Review task action, trigger, author, execution account, and task path

Failed authentication

Mistyped passwords, stale credentials, mapped drives

Correlate user, source host, frequency, lockouts, and successful logins
...
WS01 domain enrollment and domain authentication
...
Event ID 4740 lockout auditing
...
Central collection from Windows hosts

Passed

Sysmon installation and centralized forwarding

Passed

Additional Windows audit policies

Passed

Five practical detection searches
...
Two scheduled alerts enabled

Passed

Implementation Checklist

Build an isolated VirtualBox network

Deploy the corp.local Active Directory domain

Create domain users and organizational units

Join WS01 to the domain and validate authentication

Test account lockout and examine Event ID 4740

Enumerate domain-controller services and validate SMB signing

Deploy Splunk Enterprise and Universal Forwarders

Forward Windows logs from DC01 and WS01

Install, configure, and centrally forward Sysmon

Enable additional audit and PowerShell logging policies

Perform controlled security simulations

Create and test detection reports

Add detection panels to the dashboard

Configure and verify scheduled alerts
...
The lab uses one monitored workstation and a small dataset; production deployment would require tuning and baselining across more hosts.
